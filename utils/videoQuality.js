...
//   - whether the item is from Pexels (already pre-picked at adapter time)
//
 ...
 *
 * @param {object} item    - Video item (own upload or Pexels)
 * @param {object} [opts]
...
export const pickVideoUrl = (item, opts = {}) => {
  if (!item) return '';
  // Pexels items pick their resolution at adapter time — just trust videoUrl.
  if (item.source === 'pexels') return item.videoUrl || item.uri || '';
